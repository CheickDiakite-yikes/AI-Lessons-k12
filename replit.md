...
- **Enhanced multi-day lesson cohesion**: Multi-day prompts now include pedagogical scaffolding framework (Bloom's taxonomy progression), tiered by plan length: 1-week plans get a simple arc (Introduce→Practice→Apply→Assess), 2-4 week plans get weekly phases with micro-arcs, quarter/semester plans get thematic sub-units with cross-unit progression. Each daily section now requires: Connection to Prior Learning warm-up, Bloom's Level label, "Builds On" connection sentence, Vocabulary Focus (new + review terms), Exit Ticket, and Preview of next day. Spiral review required every 5th day for longer plans. Cumulative vocabulary threading across all days.
